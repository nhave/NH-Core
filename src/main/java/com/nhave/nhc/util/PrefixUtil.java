...
public class PrefixUtil
{
	...
	public static String getDisplay(double value, int decimalPlaces, boolean isShort)
	{
		...
		else
		{
			for(int i = 0; i < MeasurementUnit.values().length; i++)
			{
				...
				{
					return prefix + roundDecimals(lowerMeasure.process(value), decimalPlaces) + lowerMeasure.getName(isShort);
				}
	...
	{
		return getDisplay(value, 2, true).replaceAll(".0", "");
	}

	public static String getDisplayShort(double value, int decimalPlaces)
	{
		return getDisplay(value, decimalPlaces, true).replaceAll(".0", "");
	}
